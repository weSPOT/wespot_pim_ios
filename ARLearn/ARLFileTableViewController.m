...
- (void)viewDidLoad
{
    ...
    NSLog(@"[%s] doc dir %@", __func__, documentsDirectory);
    
    NSString * newString = [NSString stringWithFormat:@"%@/games", documentsDirectory ];
    [DownloadHelper sharedInstance].filePath = newString;
//    NSLog(@"[%s] doc dir %@", __func__, newString);
//    NSLog(@"[%s] succeeded %d", __func__, [fileMrg createDirectoryAtPath:newString withIntermediateDirectories:YES attributes:nil error:nil]);
    NSArray * array = [fileMrg contentsOfDirectoryAtPath:[paths objectAtIndex:0] error:nil];
    NSLog(@"[%s] gettting rid of warning %@", __func__, array.description);
//    int i;
...
//    
    ...
    for (i = 0; i < [subpaths count]; i++) {
        ...
        NSLog(@"[%s] sub path dir %@", __func__, [subpaths objectAtIndex:i]);
    }
