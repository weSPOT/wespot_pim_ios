...
- (void)setupFetchedResultsController {
    ...
    if (self.inquiry && self.inquiry.run && self.inquiry.run.runId) {
        ...
        NSPredicate *andPredicate = [NSPredicate predicateWithFormat: @"run.runId = %lld AND generalItem.generalItemId = %lld AND revoked != %d",[self.inquiry.run.runId longLongValue], [self.generalItem.generalItemId longLongValue], YES];
        ...
        request.predicate = [NSPredicate predicateWithFormat:
                             @"account.localId = %@ AND account.accountType = %@ AND contentType !=nil AND responseType!= %@ AND revoked != %d",
                             self.account.localId, self.account.accountType, [NSNumber numberWithInt:UNKNOWN], YES];
